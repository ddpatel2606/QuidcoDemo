...
import com.dixitpatel.quidcodemo.constant.USER_GET_TOP_ALBUMS
...
import com.dixitpatel.quidcodemo.ui.albums.model.Album
...
class AlbumsFragment : BaseFragment<AlbumsViewModel>() , SwipeRefreshLayout.OnRefreshListener {

    companion object {
        fun newInstance() = AlbumsFragment()
        var TAG = AlbumsFragment::class.toString()
    }
    ...
    lateinit var mAdapter: CommonAdapter<Album?>

    @Inject
    lateinit var apiInterface: ApiInterface

    @Inject
    lateinit var models : AlbumsViewModel
    ...
    override fun setUserVisibleHint(visible: Boolean) {
        ...
        if (this.isVisible) {
            if (visible && !hasBeenVisibleOnce) {
                ...
                {
                    apiInterface.let { models.albumApiCall(
                        USER_GET_TOP_ALBUMS,
                ...
                when (result.status) {
                    ...
                    AuthStatus.SUCCESS -> {
                        ...
                        binding.progressBar.visibility = View.GONE
                            ...
                            if(result.data!!.topalbums!!.album!!.isNotEmpty())
                            {

                                //setAdapter(result.data.topalbums!!.album)
                                //binding.tvNoResultTxt.visibility = View.GONE

                                    if (result.data.topalbums?.attr!!.page!!.toInt() != result.data.topalbums.attr.totalPages!!.toInt()) {
                                        isNextPage = true
                                        page += 1
                                    } else {
                                        isNextPage = false
                                    ...
                                    val arrData: ArrayList<Album?> = result.data.topalbums.album!!
    ...
    override fun getViewModel(): AlbumsViewModel {
        return models
    }
    ...
    private fun setAdapter(arrData: ArrayList<Album?>?) {
        mAdapter = object : CommonAdapter<Album?>(arrData!!) {
        ...
        binding.myRecyclerView.layoutManager = GridLayoutManager(requireActivity(), 2 , RecyclerView.VERTICAL,false)
