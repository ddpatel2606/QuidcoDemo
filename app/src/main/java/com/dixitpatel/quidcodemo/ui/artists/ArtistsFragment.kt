...
import com.dixitpatel.quidcodemo.constant.USER_GET_TOP_ARTIST
...
class ArtistsFragment : BaseFragment<ArtistsViewModel>(), SwipeRefreshLayout.OnRefreshListener {

    companion object {
        fun newInstance() = ArtistsFragment()
        var TAG = ArtistsFragment::class.toString()
    }
    ...
    lateinit var mAdapter: CommonAdapter<Artist?>

    @Inject
    lateinit var models : ArtistsViewModel
    ...
    override fun getViewModel(): ArtistsViewModel {
        return models
    }

    override fun setUserVisibleHint(visible: Boolean) {
        super.setUserVisibleHint(true)
            if (visible && !hasBeenVisibleOnce && isVisible) {
                if(Utils.isNetworkAvailable(requireActivity()))
                {
                    apiInterface.let { models.artistsApiCall(
                        USER_GET_TOP_ARTIST,
  ...
  override fun onActivityCreated(savedInstanceState: Bundle?) {
      ...
      setUserVisibleHint(true)
        ...
        models.artistsApiResult().observe(
            ...
            { result ->
                ...
                when (result.status) {
                    ...
                    AuthStatus.SUCCESS -> {
                        ...
                        if (result.data?.topArtists!!.artist!!.isNotEmpty()) {

                            if (result.data.topArtists.attr!!.page!!.toInt() != result.data.topArtists.attr.totalPages!!.toInt()) {
                                isNextPage = true
                                page += 1
                            } else {
                                isNextPage = false
                            ...
                            val arrData: ArrayList<Artist?> = result.data.topArtists.artist!!
    ...
    override fun onRefresh() {
        try {
            ...
            if(Utils.isNetworkAvailable(requireActivity())) {
                apiInterface.let {
                    models.artistsApiCall(
                        USER_GET_TOP_ARTIST,
                        Preferences.getPreferenceString(PrefEntity.USER_NAME)!!,
                        "json",page,
                        apiInterface
                    ) }
    ...
    private fun setAdapter(arrData: ArrayList<Artist?>) {
        mAdapter = object : CommonAdapter<Artist?>(arrData) {
